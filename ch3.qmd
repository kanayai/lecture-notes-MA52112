...
2.  An investor is considering whether or not to buy certain risky
    ...
    a.  Define appropriate actions, parameter, and parameter space for
        the problem.

    b.  Derive the loss function for the problem.

    c.  Describe all randomised decision rules and find the minimax
        decision among them.
...
3.  A coin has probability $\theta \in [0,1]$ of coming up heads
    ...
    c.  Show that the loss function, $L(\theta,a)$, for this problem is
        given by $$L(\theta,a) =
              \begin{cases}
    ...
    4.  Let $x$ be the outcome the coin flip from an earlier game.
        ...
        a.  Write a mathematical expression for the decision rules
            corresponding to these two strategies.

        b.  Between the two strategies, which one is the minimax
            decision rule?
